...
export function CurrencyTicker() {
  ...
  return (
    <div className="w-full bg-[var(--surface)] border-t border-b border-[var(--border)] overflow-hidden">
        ...
        <div className="flex items-center gap-6 overflow-x-auto scrollbar-none flex-1">
          {CURRENCY_RATES.map((rate) => (
            <div key={rate.code} className="flex items-center gap-2 flex-shrink-0">
              ...
              <span className="text-xs text-emerald-400">▲ {rate.buy.toFixed(rate.code === 'KZT' ? 3 : 2)}</span>
